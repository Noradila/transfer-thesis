...
All layers of the netstack operate on the packet buffer. One buffer holds a single packet. The packet buffer only holds the current packet.
...
Application can use either, both or none communication stack. uIP can run over Rime. Rime can run over uIP. 
...
The slip-radio resets to it's listening channel after the packet is transmitted.
