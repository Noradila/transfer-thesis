...
Contiki is defined (configured) by four layers network stack: the network layer, the MAC layer, the radio duty cycling (RDC) layer and the radio layers. The network layer includes support for TCP and UDP, IPv6, IPv4, RPL routing protocol and 6LoWPAN. Contiki's configuration option (concept) for communications, buffer management and interface (bridging) are explained before looking into MCRP implementation for ease of reading.
