...
%%Energy consumption of a node depends on the interference patterns. 
%Developed a generic method for capturing interference characteristics and predicting a node's energy consumption. 
...
%\cite{energyrpl} is an objective function for RPL that used node remaining energy as metric in the parent selection process. 
%Energy-based OF characteristics in terms of node battery level estimation, path cost and node rank computation. 
%A node selects the neighbor that advertises the greatest path cost value as parent. 
%Compute the path cost (from node i to the sink) as the minimum node energy level (between parent path cost and its own energy). 
%Uses the node's remaining energy as the main routing metric. 
%The implementation makes use of a well-known battery theoretical model which they estimate at runtime the node battery lifetime for routing. 
