...
\begin{enumerate}[i. ]
\item \textbf{Channel assignment} - Sensors have limited memory and battery capabilities. In order to maximise the sensors lifetime, a centralised LPBR that has larger memory and fully powered is used for decision making. LPBR has complete knowledge of the topology which enables it to make good channel assignment decisions based on a two-hop colouring algorithm.

%centralized; thus nodes computation is transferred to LPBR.

\item \textbf{Interference} - External interference cannot be predicted, thus channels cannot be allocated beforehand as it varies over time and locations. It is impossible to determine a single channel that is free from interference at any location. The protocol checks the channel condition each time before deciding on a channel change to reduce interference and maximise throughput. 
...
\begin{figure}
\centering
\includegraphics[trim=2cm 2cm 2cm 2cm, clip=true, totalheight=0.5\textheight, angle=270]{channelSelection.pdf}
